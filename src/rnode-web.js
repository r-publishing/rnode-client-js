...
const makeRNodeHttpInternal =
  (domFetch) => async (httpUrl, apiMethod, data) => {
...
const sendDeploy = (rnodeHttp) => async (node, account, code, phloLimit) => {
  ...
  if (!account.privKey) {
    ...
    if (ethDetected && !!ethAddr) {
      ...
      throw Error(
        `Selected account doesn't have private key and cannot be used for signing.`
      );
    }
  }
...
const getDataForDeploy =
  (rnodeHttp) =>
  async ({ httpUrl }, deployId, onProgress) => {
    ...
    const getData = (resolve, reject) => async () => {
      const getDataUnsafe = async () => {
        ...
        if (deploy) {
          ...
          if (!cancel) {
            GET_DATA_TIMEOUT_HANDLE && clearTimeout(GET_DATA_TIMEOUT_HANDLE);
            GET_DATA_TIMEOUT_HANDLE = setTimeout(
