...
const mainCtrl = (st, effects) => {
  ...
  const onCheckBalance = (node) => (revAddr) =>
    appCheckBalance({ node, revAddr });
  ...
  return m(
    ...
    hfMsgOn &&
      m(
        ...
        { style: `zoom: ${zoom}` },
        m.trust(
          "<b>Main net</b> is back online after the <b>Hard Fork 1</b>. "
        ),
        m.trust(
          'See <a target="_blank" href="https://github.com/rchain/rchip-proposals/issues/42">RCHIP#42</a> for more info.'
        )
      ),
    ...
    // REV wallet control
    addressCtrl(addressSt, {
      wallet,
      node: valNodeUrls,
      onAddAccount: onSaveAccount,
    }),

    // Check balance control
    balanceCtrl(balanceSt, {
      wallet,
      node: valNodeUrls,
