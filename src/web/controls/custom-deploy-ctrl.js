...
const initSelected = (st, wallet /** @param any[] */) => {
  ...
  const initRevAddr =
    R.isNil(selRevAddr) && !R.isNil(wallet) && !!wallet.length
      ? R.head(wallet).revAddr
      : selRevAddr;

  return { ...st, selRevAddr: initRevAddr, phloLimit };
};

export const customDeployCtrl = (
  st,
  { wallet = [], node, onSendDeploy, onPropose, warn }
) => {
  ...
  return m(
    ".ctrl.custom-deploy-ctrl",
    m("h2", "Custom deploy"),
    isWalletEmpty
      ? m(
          "b",
          `${node.tokenName} wallet is empty, add accounts to make deploys.`
        )
      : [
          ...
          m("textarea.deploy-code", {
            value: code,
            rows: 13,
            placeholder: "Rholang code",
            oninput: valEv("code"),
          }),
          ...
          m(
            "button",
            { onclick: onSendDeployEv(code), disabled: !canDeploy },
            "Deploy Rholang code"
          ),
          status && m("b", status),
          ...
          showPropose &&
            proposeError &&
            m("b.warning", showNetworkError(proposeError)),
        ]
  );
};
