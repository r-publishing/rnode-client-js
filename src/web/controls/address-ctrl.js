...
export const addressCtrl = (st, { wallet, node, onAddAccount }) => {
  ...
  return m(
    ...
    m(
      "h2",
      `${tokenName} wallet (import ${tokenName} address, ETH address, public/private key, Metamask)`
    ),
    description,
    ...
    m("input[type=text]", {
      autocomplete: "nono",
      placeholder: labelSource,
      value: text,
      oninput: addrKeyPressEv,
    }),
    ...
    ethDetected &&
      m(
        "button",
        {
          title: metamaskTitle,
          disabled: isEdit,
          onclick: fillMetamaskAccountEv,
        },
        "Metamask account"
      ),
    m(
      "button",
      { title: newAccountTitle, disabled: isEdit, onclick: newRevAddrEv },
      "New account"
    ),
    ...
    isEdit &&
      m(
        ...
        // Action buttons
        m("input[type=text].addr-name", {
          placeholder: namePlaceholder,
          value: name,
          oninput: nameKeyPressEv,
        }),
        m(
          "button.add-account",
          { title: saveTitle, onclick: addAccount, disabled: addDisabled },
          "Save account"
        ),
    ...
    wallet &&
      ...
      m(
        ...
        wallet.map(
          ({ name, privKey = "", pubKey = "", ethAddr = "", revAddr }) => {
            ...
            return m(
              ...
              m(
                ...
                priv
                  ? m(
                      "span",
                      { title: "Private key saved with this account" },
                      "✓"
                    )
                  : ""
              )
