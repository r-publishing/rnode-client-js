...
const initSelected = (st, wallet) => {
  ...
  const selToAccount =
    R.isNil(toAccount) && !R.isNil(wallet) ? R.head(wallet) : toAccount;
...
export const transferCtrl = (st, { wallet, node, onTransfer, warn }) => {
  ...
  // Control state
  const { account, toAccount, amount, status, error } = initSelected(
    st.view({}),
    wallet
  );
  ...
  const canTransfer =
    account && toAccount && amount && (account || ethDetected);
  ...
  return m(
    ...
    isWalletEmpty
      ? m("b", `${tokenName} wallet is empty, add accounts to make transfers.`)
      : [
