...
export const selectorCtrl = (st, { nets }) => {
  ...
  const findReadOnlyByIndex = (index, netName) =>
    nets
      .filter((x) => x.name === netName)
      .flatMap(({ readOnlys }) => readOnlys)[index];

  const onSelIdx = (ev) => {
    const sel = findValidatorByIndex(ev.target.selectedIndex);
    const read =
      sel.name === valNode.name ? readNode : findReadOnlyByIndex(0, sel.name);
  ...
  const isEqNode = (v1, v2) =>
    R.eqBy(
      ({ domain, gprc, https, http }) => ({ domain, gprc, https, http }),
      v1,
      v2
    );
  ...
  return m(
    ".ctrl.selector-ctrl",
    // Validator selector
    m("h2", "RPublishing Network selector"),
        ...
        m(
          ...
          hosts.map(({ name, domain, grpc, https, http }) =>
            m(
              ...
              {
                title,
                selected: isEqNode(valNode, { domain, grpc, https, http }),
              },
    ...
    m(
      "table",
      valUrls.grpcUrl &&
        m("tr", m("td", "gRPC"), m("td", m("pre", valUrls.grpcUrl))),
      m("tr", m("td", "HTTP"), m("td", m("pre", valUrls.httpUrl))),
      isLocal &&
        m("tr", m("td", "Admin"), m("td", m("pre", valUrls.httpAdminUrl)))
    ),
    isMainnet && [
      m(
        "p.warning",
        `You are connected to RPC Main network. Any deploy will use REAL ${valUrls.tokenName}s.`
      ),
    ],
    ...
    m(
      "select",
      { onchange: onSelReadIdx },
      nets
        .filter((x) => x.name === valNode.name)
        .map(({ title, readOnlys }) =>
          m(
            "optgroup",
            { label: title },
            readOnlys.map(({ name, domain, grpc, https, http }) =>
              m(
                ...
                {
                  title,
                  selected: isEqNode(readNode, { domain, grpc, https, http }),
                },
                getDdlText({ name, domain, grpc, https, http })
              )
            )
    ...
    m(
      ...
      isLocal &&
        m("tr", m("td", "Admin"), m("td", m("pre", readUrls.httpAdminUrl)))
    )
  );
};
