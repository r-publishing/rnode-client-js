...
const initSelected = (st, wallet) => {
  ...
  const selAccount =
    R.isNil(account) && !R.isNil(wallet) ? R.head(wallet) : account;
...
export const balanceCtrl = (st, { wallet = [], node, onCheckBalance }) => {
  ...
  const checkBalanceEv = async (_) => {
    ...
    const [bal, dataError] = await onCheckBalance(account.revAddr).catch(
      (ex) => ["", ex.message]
    );
    ...
    const dataBal =
      typeof bal === "number"
        ? bal === 0
          ? `${bal}`
          : `${bal} (${showTokenDecimal(bal, tokenDecimal)} ${tokenName})`
        : "";
    st.update((s) => ({ ...s, dataBal, dataError }));
  };
  ...
  return m(
    ...
    isWalletEmpty
      ? m("b", `${tokenName} wallet is empty, add accounts to check balance.`)
      : [
          ...
          m(
            "button",
            { onclick: checkBalanceEv, disabled: !account },
            "Check balance"
          ),
          m("b", dataBal),
