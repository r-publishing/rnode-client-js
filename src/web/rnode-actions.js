...
const appCheckBalance =
  ({ rnodeHttp }) =>
  async ({ node, revAddr }) => {
    const deployCode = checkBalance_rho(revAddr);
    const {
      expr: [e],
    } = await rnodeHttp(node.httpUrl, "explore-deploy", deployCode);
...
const appTransfer =
  (effects) =>
  async ({ node, fromAccount, toAccount, amount, setStatus }) => {
    ...
    const [success, message] = args || [
      false,
      "deploy found in the block but failed to get confirmation data",
    ];

    if (!success)
      throw Error(`Transfer error: ${message}. // cost: ${costTxt}`);
    return `✓ ${message} // cost: ${costTxt}`;
  };

const appSendDeploy =
  (effects) =>
  async ({ node, code, account, phloLimit, setStatus }) => {
